...
SQL="output.sql"
echo '' > ${SQL}
#CRYPT_PASSWD=$(doveadm pw -s SSHA512 -p ${DEFAULT_PASSWD})

# Cyrpt the password.
if [ X"${USE_DEFAULT_PASSWD}" == X"YES" ]; then
#    export CRYPT_PASSWD="$(doveadm pw -s SSHA512 -p ${DEFAULT_PASSWD})"
     CRYPT_PASSWD=`doveadm pw -s SSHA512 -p 'Vinahost@888'`
else
    :
fi
...
INSERT INTO mailbox (username, password, name, storagebasedirectory,storagenode, maildir, quota, domain, active, created)
    VALUES ('${mail}', '${CRYPT_PASSWD}', '${username}', '${STORAGE_BASE}','${STORAGE_NODE}', '${maildir}', '${DEFAULT_QUOTA}', '${DOMAIN}', '1', NOW());
INSERT INTO alias (address, domain, created, active) VALUES ('${mail}','${DOMAIN}', NOW(), 1);
EOF
    done
}
...
SQL template file was generated successfully, Please check it
*MANUALLY* after verify the records:
